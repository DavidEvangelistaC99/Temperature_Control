...
void regla_compuesta3(float in1[],float in2[],float in3[],float U1[],float U2[],float U3[],float c1,float c2,float c3, float V[], float UV[], float B[], int tam){
  ...
  while(date ==0){
    while(date1==0){
      ...
      t3=t3+1;	//Define size in3
      }
    if(in2[t2]=='T' || in2[t2]=='t' || in2[t2]=='R' || in2[t2]=='r'){
        date1=1;
        t2=t2-1;
        }
      t2=t2+1;	//Define size in2
    }
    if(in1[t1]=='T' || in1[t1]=='t' || in1[t1]=='R' || in1[t1]=='r'){
        date=1;
        t1=t1-1;
        }
      t1=t1+1;	//Define size in1
  }
  //Define size input vectors
  if(t1==3){
    r1 = triangular(U1,in1,c1);
  }
    else if(t1==4){
...
void inferencia_mamdani2(float B[],float c,float stru[],float U_VOL[],int tama){ //Mamdani Inference
  ...
  float v[tama]; // Tamaño del vector del universo discurso de voltaje
